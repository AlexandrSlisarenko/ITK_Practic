package ru.slisarenko.persist.config;
...
import ru.slisarenko.persist.exception.NonRetryableException;
import ru.slisarenko.persist.exception.RetryableException;
...
public class KafkaConfig {
    ...
    @Bean
    public ProducerFactory<String, PersistDTO> producerFactoryOrder() {
        return new DefaultKafkaProducerFactory<>(buildProducerConfigs());
    }
    ...
    @Bean
    public KafkaTemplate<String, PaymentRequestDTO> kafkaTemplatePayment() {
        return new KafkaTemplate<>(producerFactoryPayment());
    }

    @Bean
    public KafkaTemplate<String, ShippingRequestDTO> kafkaTemplateShipping() {
    ...
    NewTopic createRequestTopic() {
        return TopicBuilder.name(SENT_PERSIST_REQUEST_TOPIC)
                .partitions(3)
                .replicas(3)
    ...
    NewTopic createResponseTopic() {
        return TopicBuilder.name(SENT_PERSIST_RESPONSE_TOPIC)
                .partitions(3)
                .replicas(3)
                .configs(Map.of("min.insync.replicas",
                        Objects.requireNonNull(environment.getProperty("spring.kafka.producer.properties.min.insync.replicas"))))
